...
const patterns = {
  ...
  GameRoundWinPattern:
    /Team '(\w+)' triggered 'SFUI_Notice_(\w+)' \(CT '(\d+)'\) \(T '(\d+)'\)/,
  GameScorePattern:
    /MatchStatus: Score: (\d+):(\d+) on map '[\w:]+' RoundsPlayed: (\d+)/,
  GamePausePattern: /Match pause is (enabled|disabled) - ([\w:]+)/,
  FaceItScorePattern: /(?:\^])\s+([^[]+)\s+\[(\d+)\s+-\s+(\d+)\]\s+([^[]+)/,
  PlayerDroppedPattern:
    /'(.+)<(\d+)><([\w:]+)><(TERRORIST|CT)>' dropped '(\w+)'/,
  PlayerPickedUpPattern:
    /'(.+)<(\d+)><([\w:]+)><(TERRORIST|CT)>' picked up '(\w+)'/,
  PlayerKillPattern:
    /'(.+)<(\d+)><([\w:]+)><(TERRORIST|CT)>' \[(-?\d+) (-?\d+) (-?\d+)\] killed '(.+)<(\d+)><([\w:]+)><(TERRORIST|CT)>' \[(-?\d+) (-?\d+) (-?\d+)\] with '(\w+)' ?(\(?(headshot|penetrated|headshot penetrated)?\))?/,
  PlayerKillAssistPattern:
    /'(.+)<(\d+)><([\w:]+)><(TERRORIST|CT)>' (flash-)?assisted killing '(.+)<(\d+)><([\w:]+)><(TERRORIST|CT)>'/,
  PlayerAttackPattern:
    /'(.+)<(\d+)><([\w:]+)><(TERRORIST|CT)>' \[(-?\d+) (-?\d+) (-?\d+)\] attacked '(.+)<(\d+)><([\w:]+)><(TERRORIST|CT)>' \[(-?\d+) (-?\d+) (-?\d+)\] with '(\w+)' \(damage '(\d+)'\) \(damage_armor '(\d+)'\) \(health '(\d+)'\) \(armor '(\d+)'\) \(hitgroup '([\w ]+)'\)/,
  PlayerThrewPattern:
    /'(.+)<(\d+)><([\w:]+)><(TERRORIST|CT)>' threw (\w+) \[(-?\d+) (-?\d+) (-?\d+)\]( flashbang entindex (\d+))?\)?/,
  PlayerBlindedPattern:
    /'(.+)<(\d+)><([\w:]+)><(TERRORIST|CT)>' blinded for ([\d.]+) by '(.+)<(\d+)><([\w:]+)><(TERRORIST|CT)>' from flashbang entindex (\d+)/,
  PlayerMoneyChangePattern:
    /'(.+)<(\d+)><([\w:]+)><(TERRORIST|CT)>' money change (\d+)\+?(-?\d+) = \$(\d+) \(tracked\)( \(purchase: (\w+)\))?/,
  PlayerMoneyHasPattern: /'(.+)<(\d+)><([\w:]+)><(TERRORIST|CT)>' has (\d+)\$/,
  PlayerPurchasePattern:
    /'(.+)<(\d+)><([\w:]+)><(TERRORIST|CT)>' purchased '(\w+)'/,
  PlayerLeftBuyzonePattern:
    /'(.+)<(\d+)><([\w:]+)><(TERRORIST|CT)>' left buyzone with \[ (.*) \]/,
  PlayerBombGotPattern:
    /'(.+)<(\d+)><([\w:]+)><(TERRORIST|CT)>' triggered 'Got_The_Bomb'/,
  PlayerBombBeginPlantPattern:
    /'(.+)<(\d+)><([\w:]+)><(TERRORIST|CT)>' triggered 'Bomb_Begin_Plant' at bombsite ([A-Za-z])/,
  PlayerBombPlantedPattern:
    /'(.+)<(\d+)><([\w:]+)><(TERRORIST|CT)>' triggered 'Planted_The_Bomb' at bombsite ([A-Za-z])/,
  PlayerBombDroppedPattern:
    /'(.+)<(\d+)><([\w:]+)><(TERRORIST|CT)>' triggered 'Dropped_The_Bomb'/,
  PlayerBombBeginDefusePattern:
    /'(.+)<(\d+)><([\w:]+)><(TERRORIST|CT)>' triggered 'Begin_Bomb_Defuse_With(out)?_Kit'/,
  PlayerBombDefusedPattern:
    /'(.+)<(\d+)><([\w:]+)><(TERRORIST|CT)>' triggered 'Defused_The_Bomb'/,
  TeamScoredPattern:
    /Team '(CT|TERRORIST)' scored '(\d+)' with '(\d+)' players/,
